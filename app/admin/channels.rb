...
ActiveAdmin.register Channel do
  permit_params :client_id, :device_id, :category, :platform, :path, :tipo, :color, :range, :array_info, :label, :previous_state, :obs
  ...
  index pagination_total: false do
    ...
    column :category do |channel|
      div class: (channel.category == 'publish' ? 'publish' : 'subscrible').to_s do
        channel.category
      end
    end
    column :path
    ...
    column :created_at
    column :updated_at
    column :obs do |channel|
      channel.obs&.slice(0, 50)
    end
  end

  batch_action :Criar_Channels_para_Mini_Monitor, form: { client_id: :text, device_id: :text, path: :text } do |_ids, inputs|
    Channel.create(client_id: inputs['client_id'], device_id: inputs['device_id'], path: "#{inputs['path']}ativo",        category: 'subscrible', tipo: 'ativo')
    Channel.create(client_id: inputs['client_id'], device_id: inputs['device_id'], path: "#{inputs['path']}terminal_OUT", category: 'subscrible', tipo: 'terminal_view')
    Channel.create(client_id: inputs['client_id'], device_id: inputs['device_id'], path: "#{inputs['path']}prefes_OUT",   category: 'subscrible', tipo: 'prefes_view')
    Channel.create(client_id: inputs['client_id'], device_id: inputs['device_id'], path: "#{inputs['path']}info",         category: 'subscrible', tipo: 'info',      color: 'dark')
    Channel.create(client_id: inputs['client_id'], device_id: inputs['device_id'], path: "#{inputs['path']}vcc1",         category: 'subscrible', tipo: 'value',     color: 'primary', label: 'Tensão VCC',            previous_state: 'Aguardando...')
    Channel.create(client_id: inputs['client_id'], device_id: inputs['device_id'], path: "#{inputs['path']}led2",         category: 'subscrible', tipo: 'led',       color: 'primary', label: 'Tensão VAC',            previous_state: '0', range: '0-1', array_info: 'Falha-OK')
    Channel.create(client_id: inputs['client_id'], device_id: inputs['device_id'], path: "#{inputs['path']}led1",         category: 'subscrible', tipo: 'led',       color: 'primary', label: 'Sensor de Abertura',    previous_state: '0', range: '0-1', array_info: 'Aberto-Fechado')
    Channel.create(client_id: inputs['client_id'], device_id: inputs['device_id'], path: "#{inputs['path']}reiniciar",    category: 'publish',    tipo: 'reiniciar', color: 'danger',  label: 'Reiniciar Dispositivo', previous_state: '0', range: '0-1', array_info: 'push')
    Channel.create(client_id: inputs['client_id'], device_id: inputs['device_id'], path: "#{inputs['path']}terminal_IN",  category: 'publish',    tipo: 'terminal_insert')
    Channel.create(client_id: inputs['client_id'], device_id: inputs['device_id'], path: "#{inputs['path']}prefes_IN",    category: 'publish',    tipo: 'prefes_insert')
    Channel.create(client_id: inputs['client_id'], device_id: inputs['device_id'], path: "#{inputs['path']}update",       category: 'publish',    tipo: 'update')

    redirect_to collection_path, alert: 'Canais criados com sucesso.'
  end

  batch_action :Criar_Channels_para_Alarme, form: { client_id: :text, device_id: :text, path: :text } do |_ids, inputs|
    Channel.create(client_id: inputs['client_id'], device_id: inputs['device_id'], path: "#{inputs['path']}ativo",        category: 'subscrible', tipo: 'ativo')
    Channel.create(client_id: inputs['client_id'], device_id: inputs['device_id'], path: "#{inputs['path']}terminal_OUT", category: 'subscrible', tipo: 'terminal_view')
    Channel.create(client_id: inputs['client_id'], device_id: inputs['device_id'], path: "#{inputs['path']}prefes_OUT",   category: 'subscrible', tipo: 'prefes_view')
    Channel.create(client_id: inputs['client_id'], device_id: inputs['device_id'], path: "#{inputs['path']}info",         category: 'subscrible', tipo: 'info', color: 'dark')
    ...
    Channel.create(client_id: inputs['client_id'], device_id: inputs['device_id'], path: "#{inputs['path']}prefes_IN",    category: 'publish',    tipo: 'prefes_insert')
    Channel.create(client_id: inputs['client_id'], device_id: inputs['device_id'], path: "#{inputs['path']}reiniciar",    category: 'publish',    tipo: 'reiniciar', color: 'danger', label: 'Reiniciar Dispositivo', previous_state: '0', range: '0-1', array_info: 'push')
    Channel.create(client_id: inputs['client_id'], device_id: inputs['device_id'], path: "#{inputs['path']}rele1",        category: 'publish',    tipo: 'button',    color: 'danger', label: 'Alarme',                previous_state: '0', range: '1',   array_info: 'push')
  ...
  form do |f|
    f.inputs do
      ...
      f.input :tipo,     as: :select, collection: %w[ativo button info led slide terminal_insert terminal_view update reiniciar value prefes_insert prefes_view update]
